...
pub fn update() !quanta.app.UpdateResult {
    ...
    //update
    {
        const x_position = @as(f32, @floatCast(window.window.getCursorPos().xpos));
        const y_position = @as(f32, @floatCast(window.window.getCursorPos().ypos));
    ...
    if (true) {
        {
            ...
            if (state.selected_entities.items.len != 0 and state.ecs_scene.entityHasComponent(primary_selected_entity, quanta.components.Position)) {
                ...
                imguizmo.ImGuizmo_SetRect(0, 0, @as(f32, @floatFromInt(window.getWidth())), @as(f32, @floatFromInt(window.getHeight())));
...
pub fn _main() !void {
    ...
    const asset_archive_blob = try std.os.mmap(null, @as(usize, @intCast(asset_archive_fd_stat.size)), std.os.PROT.READ, std.os.MAP.PRIVATE, asset_archive_fd, 0);
    ...
    const test_scene_blob = asset_archive.getAssetData(@as(asset.Archive.AssetDescriptor, @enumFromInt(3)));
    ...
    const environment_map_data = asset_archive.getAssetData(@as(asset.Archive.AssetDescriptor, @enumFromInt(2)));
    ...
    while (!window.shouldClose()) {
        ...
        if (true) {
            {
                ...
                if (window.getMouseDown(.left) and !mouse_pressed_last_Frame and !imguizmo.ImGuizmo_IsUsing() and !imguizmo.ImGuizmo_IsOver() and !imgui.igIsAnyItemFocused()) {
                    ...
                    const normalized_pos = [2]f32{
                        ((mouse_pos[0] / @as(f32, @floatFromInt(window.getWidth()))) * 2) - 1,
                        (((mouse_pos[1] - @as(f32, @floatFromInt(window.getHeight()))) / @as(f32, @floatFromInt(window.getHeight()))) * 2) - 1,
                    };
        ...
        {
            delta_time = @as(f32, @floatFromInt(std.time.nanoTimestamp() - time_begin)) / @as(f32, @floatFromInt(std.time.ns_per_ms));
        }
    }
